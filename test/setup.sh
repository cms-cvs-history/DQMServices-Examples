...
echo "The scram architecture is = $SCRAM_ARCH"

TEST_PATH=$(echo "${PWD}" | sed 's/\//\\\//g')
echo "The current directory is = $PWD"

MWC_LIB1="${LOCALRT}/lib/${SCRAM_ARCH}/libDQMServicesExamples.so"
echo "Looking for the MonitorWebClient library... $MWC_LIB1"
if [ ! -f $MWC_LIB1 ]; then
    echo "Not Found! Will pick it up from the release area..."
    
MWC_LIB1="${CMSSW_RELEASE_BASE}/lib/${SCRAM_ARCH}/libDQMServicesExamples.so"
else 
    echo "Found!"
fi
...
sed -e "s/\.portn/1972/g" -e "s/\.host/${HOSTNAME}/g" -e "s/\.pwd/${TEST_PATH}/g" -e "s/.libpath/${MWC_LIB}/g" .ClientWithWebInterface.xml > ClientWithWebInterface.xml 
...
chmod 751 ClientWithWebInterface.xml
